...
const ConfigPage = () => {
	...
	useEffect(() => {
		const getData = async () => {
			await ProjectFireStore.collection('Config')
				.doc('Date')
				.get()
				.then((res) => {
					try {
			...
			>
				{Toggle && (
					<>
						...
						<div className='submitAndCancel'>
							...
							<div onClick={() => onReset()} className='CancelButton btn'>
								{' '}
								RESET
							</div>
						</div>
				...
				{!Toggle && (
					<>
						<div>
							<h3>
								START DATE:<h5>{startDateTimeStamps.toString()}</h5>
							</h3>
							<br />
							<hr />
							<br />
							<h3>
								END DATE:<h5>{endDateTimeStamps.toString()}</h5>
							</h3>
						</div>
					</>
				)}
